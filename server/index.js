...
app.get('/api/students', requireAuth, (req, res) => {
  ...
  const withStats = rows.map(s => {
    ...
    const fees = feeStmt.get(s.id);
    return {
      ...s,
      result_count: stats.c,
      average_score: stats.avg ? Math.round(stats.avg) : null,
      fee_balance: fees.charged - fees.paid
    };
  });
...
app.get('/api/students/:id/attendance', requireAuth, (req, res) => {
  const student = db.prepare('SELECT * FROM students WHERE id = ?').get(req.params.id);
  if (!student) return res.status(404).json({ error: 'Student not found.' });
  const scope = scopeFilter(req);
  if (scope && student.class !== scope) return res.status(403).json({ error: 'Not authorized for this class.' });

  const records = db.prepare('SELECT * FROM attendance WHERE student_id = ? ORDER BY date DESC').all(student.id);
  res.json(records);
});

app.post('/api/students/:id/attendance', requireAuth, (req, res) => {
  const student = db.prepare('SELECT * FROM students WHERE id = ?').get(req.params.id);
  if (!student) return res.status(404).json({ error: 'Student not found.' });
  const scope = scopeFilter(req);
  if (scope && student.class !== scope) return res.status(403).json({ error: 'Not authorized for this class.' });

  const { date, am_present, pm_present } = req.body;
  if (!date) return res.status(400).json({ error: 'Date is required.' });

  db.prepare(`
    INSERT INTO attendance (student_id, date, am_present, pm_present, recorded_by)
    VALUES (?, ?, ?, ?, ?)
    ON CONFLICT(student_id, date) DO UPDATE SET
      am_present = excluded.am_present,
      pm_present = excluded.pm_present,
      recorded_by = excluded.recorded_by
  `).run(student.id, date, am_present ? 1 : 0, pm_present ? 1 : 0, req.user.id);
...
app.get('/api/students/:id/fees', requireAuth, (req, res) => {
  const student = db.prepare('SELECT * FROM students WHERE id = ?').get(req.params.id);
  if (!student) return res.status(404).json({ error: 'Student not found.' });
  const scope = scopeFilter(req);
  if (scope && student.class !== scope) return res.status(403).json({ error: 'Not authorized for this class.' });

  const entries = db.prepare('SELECT * FROM fee_entries WHERE student_id = ? ORDER BY entry_date DESC, id DESC').all(student.id);
  const charged = entries.filter(e => e.type === 'charge').reduce((a, e) => a + e.amount, 0);
  const paid = entries.filter(e => e.type === 'payment').reduce((a, e) => a + e.amount, 0);
  res.json({ entries, totalCharged: charged, totalPaid: paid, balance: charged - paid });
});

app.post('/api/students/:id/fees', requireAuth, (req, res) => {
  const student = db.prepare('SELECT * FROM students WHERE id = ?').get(req.params.id);
  if (!student) return res.status(404).json({ error: 'Student not found.' });
  const scope = scopeFilter(req);
  if (scope && student.class !== scope) return res.status(403).json({ error: 'Not authorized for this class.' });

  const { type, amount, note, entry_date } = req.body;
  const numAmount = Number(amount);
  if (!['charge', 'payment'].includes(type) || isNaN(numAmount) || numAmount <= 0) {
    return res.status(400).json({ error: 'A valid type (charge/payment) and positive amount are required.' });
  }

  const info = db.prepare(`
    INSERT INTO fee_entries (student_id, type, amount, note, entry_date, recorded_by)
    VALUES (?, ?, ?, ?, COALESCE(?, date('now')), ?)
  `).run(student.id, type, numAmount, note || '', entry_date || null, req.user.id);

  res.json({ id: info.lastInsertRowid });
});

app.delete('/api/fees/:id', requireAuth, (req, res) => {
  const entry = db.prepare('SELECT * FROM fee_entries WHERE id = ?').get(req.params.id);
  if (!entry) return res.status(404).json({ error: 'Entry not found.' });
  const student = db.prepare('SELECT * FROM students WHERE id = ?').get(entry.student_id);
  const scope = scopeFilter(req);
  if (scope && student.class !== scope) return res.status(403).json({ error: 'Not authorized for this class.' });

  db.prepare('DELETE FROM fee_entries WHERE id = ?').run(entry.id);
  res.json({ ok: true });
});
